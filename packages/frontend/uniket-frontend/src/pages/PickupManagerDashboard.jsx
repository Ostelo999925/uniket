...
import { FaSearch, FaCheckCircle, FaTruck, FaBox, FaClock } from 'react-icons/fa';
import './PickupManagerDashboard.css';

const PickupManagerDashboard = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Calculate stats based on pickedUpAt field
      const total = response.data.length;
      const pending = response.data.filter(order => !order.pickedUpAt).length;
      const completed = response.data.filter(order => order.pickedUpAt).length;
...
export default PickupManagerDashboard; 